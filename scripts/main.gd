extends Control
## Main scene — Drone Tycoon: Sky Fleet. Portrait, premium dark UI.
...
func _ready() -> void:
	...
	GameState.milestone_reached.connect(func(m): _toast("Marco! Lucros x%d" % int(m), UITheme.GOLD))
...
func _bg() -> void:
	...
	grad.set_color(0, UITheme.BG0); grad.set_color(1, UITheme.BG1)
...
func _icon(n: String, sz := 28) -> TextureRect:
	...
	r.texture = _tex(n); r.custom_minimum_size = Vector2(sz, sz)
...
func _pill(icon: String, color: Color) -> Dictionary:
	...
	pc.add_theme_stylebox_override("panel", UITheme.pill())
	pc.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	var h := HBoxContainer.new(); h.add_theme_constant_override("separation", 6); pc.add_child(h)
	h.add_child(_icon(icon, 26))
	var lbl := _lbl("0", 27, color); lbl.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	lbl.add_theme_font_override("font", UITheme.font("Bold"))
	h.add_child(lbl)
...
func _build_hud() -> void:
	...
	var gear := Button.new(); gear.custom_minimum_size = Vector2(52, 52)
	gear.add_theme_stylebox_override("normal", UITheme.solid(UITheme.PANEL2))
	gear.add_child(_icon("ic_gear", 26)); gear.pressed.connect(_show_settings); row.add_child(gear)
	# milestone bar + income
	...
	_ms_lbl = _lbl("Próximo marco", 18, UITheme.MUTED); barbox.add_child(_ms_lbl)
	_ms_bar = ProgressBar.new(); _ms_bar.show_percentage = false; _ms_bar.min_value = 0; _ms_bar.max_value = 1; _ms_bar.custom_minimum_size = Vector2(0, 14)
	_ms_bar.add_theme_stylebox_override("background", UITheme.pill(Color(0.1, 0.13, 0.22)))
	_ms_bar.add_theme_stylebox_override("fill", UITheme.solid(UITheme.GOLD, 10))
	barbox.add_child(_ms_bar); row2.add_child(barbox)
	var inc := VBoxContainer.new(); inc.add_theme_constant_override("separation", 0)
	_income_lbl = _lbl("+0/s", 26, UITheme.GREEN); _income_lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_RIGHT
	_income_lbl.add_theme_font_override("font", UITheme.font("Bold")); inc.add_child(_income_lbl)
	row2.add_child(inc)
...
func _build_boost_bar() -> void:
	...
	_boost_bar.offset_left = 8; _boost_bar.offset_right = -8; _boost_bar.add_theme_constant_override("separation", 8)
...
func _green_btn(icon: String, text: String, cb: Callable) -> Button:
	...
	b.size_flags_horizontal = Control.SIZE_EXPAND_FILL; b.add_theme_font_size_override("font_size", 21)
	b.add_theme_stylebox_override("normal", UITheme.solid(UITheme.GREEN))
	b.add_theme_stylebox_override("hover", UITheme.solid(UITheme.GREEN.lightened(0.08)))
	b.add_theme_stylebox_override("pressed", UITheme.solid(UITheme.GREEN_D))
	b.add_child(_icon(icon, 24)); b.icon_alignment = HORIZONTAL_ALIGNMENT_LEFT; b.pressed.connect(cb)
	return b
...
func _build_tabs() -> void:
	...
	_tabs.add_child(_build_talents_tab())
...
func _build_fleet_tab() -> ScrollContainer:
	...
	for m in [[1, "x1"], [10, "x10"], [100, "x100"], [-1, "Máx"]]:
		var b := Button.new(); b.text = m[1]; b.size_flags_horizontal = Control.SIZE_EXPAND_FILL; b.add_theme_font_size_override("font_size", 21)
		b.add_theme_stylebox_override("normal", UITheme.solid(UITheme.PANEL2))
		b.pressed.connect(func(): GameState.buy_mode = m[0]); modes.add_child(b); _mode_btns[m[0]] = b
	# drone card
	...
	left.add_child(_lbl("Comprar Drone", 25, UITheme.INK))
	_drone_detail = _lbl("", 19, UITheme.MUTED); left.add_child(_drone_detail)
...
func _build_network_tab() -> ScrollContainer:
	...
	_hub_detail = _lbl("", 19, UITheme.MUTED); _hub_detail.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART; left.add_child(_hub_detail)
	...
	v.add_child(_lbl("Cada cidade ligada dá +50% de lucro de rede\ne abre uma nova rota de entregas.", 19, UITheme.MUTED))
	return r[0]

func _build_talents_tab() -> ScrollContainer:
	var r := _scroll("Talentos"); var v: VBoxContainer = r[1]
	v.add_child(_lbl("Gasta Influência (do prestígio) em bónus PERMANENTES.", 20, UITheme.MUTED))
...
func _build_shop_tab() -> ScrollContainer:
	...
	v.add_child(_lbl("Compras de demonstração (sem pagamento real).\nProntas para Google Play Billing.", 18, UITheme.MUTED))
...
func _build_prestige_tab() -> ScrollContainer:
	...
	v.add_child(_lbl("Expandir a Rede reinicia a operação mas dá Influência\npermanente (+15% lucros cada) para gastar em Talentos.", 21, UITheme.INK))
	_prestige_info = _lbl("", 24, UITheme.VIOLET); v.add_child(_prestige_info)
	_prestige_btn = Button.new(); _prestige_btn.text = "Expandir a Rede"; _prestige_btn.add_theme_font_size_override("font_size", 26)
	_prestige_btn.add_theme_stylebox_override("normal", UITheme.solid(UITheme.VIOLET))
	_prestige_btn.add_theme_stylebox_override("hover", UITheme.solid(UITheme.VIOLET.lightened(0.08)))
	_prestige_btn.pressed.connect(func(): _confirm.popup_centered()); v.add_child(_prestige_btn)
	return r[0]

func _build_confirm() -> void:
	_confirm = ConfirmationDialog.new()
	_confirm.title = "Expandir a Rede"; _confirm.dialog_text = "Reiniciar a operação por Influência?"
	_confirm.confirmed.connect(func(): GameState.do_prestige()); add_child(_confirm)

func _build_toasts() -> void:
	_toasts = VBoxContainer.new()
	_toasts.anchor_left = 0.5; _toasts.anchor_right = 0.5; _toasts.offset_top = 150; _toasts.offset_left = -280; _toasts.offset_right = 280
...
func _buy_btn() -> Button:
	...
	b.add_theme_stylebox_override("hover", UITheme.solid(UITheme.GREEN.lightened(0.08)))
	b.add_theme_stylebox_override("pressed", UITheme.solid(UITheme.GREEN_D))
	b.add_theme_stylebox_override("disabled", UITheme.solid(Color(0.2, 0.24, 0.34)))
	return b
...
func _make_upgrade_row(key: String) -> PanelContainer:
	...
	var detail := _lbl("", 19, UITheme.MUTED); left.add_child(detail)
...
func _make_talent_row(key: String) -> PanelContainer:
	...
	var detail := _lbl("", 18, UITheme.MUTED); left.add_child(detail)
	var btn := Button.new(); btn.custom_minimum_size = Vector2(150, 66); btn.add_theme_font_size_override("font_size", 21)
	btn.add_theme_stylebox_override("normal", UITheme.solid(UITheme.VIOLET))
	btn.add_theme_stylebox_override("disabled", UITheme.solid(Color(0.2, 0.24, 0.34)))
	btn.pressed.connect(func(): if GameState.buy_talent(key): _pulse(btn); Audio.play("buy"))
	h.add_child(btn); _talent_rows[key] = {"btn": btn, "detail": detail}
	return panel
...
func _make_shop_row(id: String) -> PanelContainer:
	...
	btn.pressed.connect(func(): Billing.buy(id); Audio.play("buy")); h.add_child(btn)
	return panel
...
func _process(delta: float) -> void:
	...
	var step: int = Economy.MILESTONE_STEP
	var into := GameState.drones % step
	_ms_bar.value = float(into) / float(step)
	_ms_lbl.text = "Lucros x%d  ·  próximo x2 em %d drones" % [int(GameState.milestone_mult()), step - into]
	...
	_drone_detail.text = "Tens %d · x2 a cada %d" % [GameState.drones, Economy.MILESTONE_STEP]
	...
	for key in _talent_rows:
		var tp: Dictionary = Economy.TALENTS[key]
		var lvl := int(GameState.talents[key]); var tr: Dictionary = _talent_rows[key]
		if lvl >= int(tp["max"]):
			tr["btn"].text = "MÁX"; tr["btn"].disabled = true
		else:
			tr["btn"].text = str(GameState.talent_cost(key)) + " Inf"
			tr["btn"].disabled = not GameState.can_buy_talent(key)
	...
	_prestige_info.text = "Expandir agora: +%s Influência   ·   Lucros x%.2f" % [Fmt.short(gain), GameState.global_mult()]
	_prestige_btn.disabled = gain < 1

func _effect(key: String) -> String:
	match key:
		"speed": return "+6% por drone"
		"cargo": return "+0.5 carga"
		"value": return "+8% valor"
	return ""
...
func _toast(text: String, accent: Color) -> void:
	var l := _lbl(text, 27, Color.WHITE); l.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	var pc := PanelContainer.new(); pc.add_theme_stylebox_override("panel", UITheme.solid(accent, 16)); pc.add_child(l)
	_toasts.add_child(pc); pc.modulate = Color(1, 1, 1, 0)
	var tw := create_tween()
	tw.tween_property(pc, "modulate", Color(1, 1, 1, 1), 0.2); tw.tween_interval(1.9)
